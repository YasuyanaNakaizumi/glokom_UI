...
const startIdx = lines.findIndex(l => l.includes('{/* Header Row */}'));
const endIdx = lines.findIndex(l => l.includes('{/* Map Modal */}')) - 2; // Before '      </div>'

if (startIdx !== -1 && endIdx !== -1) {
  const horizontalGrid = lines.slice(startIdx, endIdx).join('\\n');
